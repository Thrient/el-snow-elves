...
const TaskSettingsModal: FC<Props> = ({ open, task, onClose }) => {
  ...
  const Row: FC<{ icon: React.ReactNode; label: string; children: React.ReactNode }> = ({
    ...
    <div className="flex items-start gap-3">
      <div className="flex items-center gap-2" style={{ width: 70, flexShrink: 0, paddingTop: 4 }}>
        ...
        <span className="text-[12px] font-medium text-[#8b8fa3] leading-tight">{label}</span>
      </div>
  ...
  return (
    <>
      ...
      {/* Modal */}
      <div
        className="fixed top-1/2 left-1/2 z-[1001] bg-white rounded-2xl shadow-xl overflow-hidden"
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[15px] font-semibold text-[#1a1a2e]">任务设置</span>
          </div>
          <button
            onClick={onClose}
            className="w-7 h-7 flex items-center justify-center rounded-lg border-0 bg-transparent text-[#c0c4cc] hover:text-[#374151] hover:bg-[#f0f2f5] transition-colors cursor-pointer shrink-0"
          >
        ...
        <div className="px-6 py-5 space-y-5 overflow-y-auto" style={{ maxHeight: "calc(100vh - 180px)" }}>
          ...
          {/* Row 4: 备注 */}
          <Row
            icon={<FileTextOutlined className="text-[#8b8fa3]" />}
            label="备注"
          >
            <Input.TextArea
